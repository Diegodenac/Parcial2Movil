...
class NavigationViewModel: ViewModel() {
            ...
            try {
                ...
                when {
                    ...
                    intent?.hasExtra("click_action") == true -> {
                        ...
                        navigateTo(Screen.DollarScreen.route, NavigationOptions.CLEAR_BACK_STACK)
                    }
                }
            } catch (e: Exception) {
    ...
    private fun handleClickAction(clickAction: String?) {
        when (clickAction) {
            "OPEN_PROFILE" -> navigateTo(Screen.ProfileScreen.route, NavigationOptions.REPLACE_HOME)
            "OPEN_MOVIES" -> navigateTo(Screen.MovieScreen.route, NavigationOptions.REPLACE_HOME)
            "OPEN_DOLLAR" -> navigateTo(Screen.DollarScreen.route, NavigationOptions.CLEAR_BACK_STACK)
            else -> navigateTo(Screen.DollarScreen.route, NavigationOptions.CLEAR_BACK_STACK)
        }
    }
    ...
    private fun handleUriDeepLink(uri: android.net.Uri?) {
        when (uri?.host) {
            "profile" -> {
                navigateTo(Screen.ProfileScreen.route, NavigationOptions.REPLACE_HOME)
            }
            "movies" -> {
                navigateTo(Screen.MovieScreen.route, NavigationOptions.REPLACE_HOME)
            }
            "dollar" -> {
                navigateTo(Screen.DollarScreen.route, NavigationOptions.CLEAR_BACK_STACK)
            }
    ...
    private fun handleNavigationDestination(destination: String?) {
        when (destination?.uppercase()) {
            "PROFILE" -> navigateTo(Screen.ProfileScreen.route, NavigationOptions.REPLACE_HOME)
            "MOVIES" -> navigateTo(Screen.MovieScreen.route, NavigationOptions.REPLACE_HOME)
            "DOLLAR" -> navigateTo(Screen.DollarScreen.route, NavigationOptions.CLEAR_BACK_STACK)
